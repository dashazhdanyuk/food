...
function calc() {
  ...
  function getStaticInformation(selector, activeClass) {
    //функция для дивов активности и выбора male/female
...
function cards() {
  ...
  class MenuCard {
    constructor(src, alt, title, descr, price, parentSeletor) {
      this.src = src;
      this.alt = alt;
      this.title = title;
      this.descr = descr;
...
function openModal(modalSelector, modalTimerId) {
  //вынесли в функцию код, который повторяется в коде больше одного раза don`t repeat yourself (dry)
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  modal.addEventListener('click', e => {
    if (e.target === modal || e.target.getAttribute('data-close') == '') {
      //если пользлватель кликает на подложку (за пределы модального окна, то окно закрывается), то есть если клик пользователя равно подложке modal, то окно скроется
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.code === 'Escape' && modal.classList.contains('show')) {
      //если пользователь нажал на esc и модальное окно открыто
      closeModal();
    }
  });
  function showModalByScroll() {
    //функционал, который открывает модальное окно в случае прокрутки сайта до конца страницы 
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
      //если прокрученная часть + видимая часть на экране юзера = высоте всей прокрутки (смотреть таблицу в тетради) 
...
function slider(_ref) {
  ...
  next.addEventListener('click', () => {
    if (offset == deleteNotDigits(width) * (slides.length - 1)) {
      //'500px' - пример переменной width, поэтому нужно обработать ее, чтобы это значение было число без px
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  tabsParent.addEventListener('click', event => {
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      //передаем селектор, но без точки (точку вырезаем)
      tabs.forEach((item, i) => {
        if (target == item) {
          hideTabContent();
          showtabContent(i);
...
function timer(id, deadline) {
  ...
  function getZero(num) {
    //добавляет ноль в числа состоящие из одной цифры 2023-01-01
    if (num >= 0 && num < 10) {
      return `0${num}`;
    } else {
      return num;
...
window.addEventListener('DOMContentLoaded', () => {
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__.openModal)('.modal', modalTimerId), 300000); //открывается модальное окно через 30 секунд перебывания пользователя на сайте 
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2023-12-11');
